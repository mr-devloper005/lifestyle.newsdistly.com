...
const inputClass = 'h-[3.5rem] rounded-[1.25rem] border border-[rgba(90,31,207,0.12)] bg-white/85 px-4 text-base font-semibold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent-fill)] focus:bg-white'
const buttonClass = 'inline-flex h-[3.5rem] items-center justify-center rounded-full bg-[var(--slot4-accent-fill)] px-6 text-xs font-black uppercase tracking-[0.2em] text-white transition hover:-translate-y-0.5 hover:bg-[var(--slot4-dark-bg)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.1rem] border px-4 py-3 text-sm font-semibold ${status === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-rose-200 bg-rose-50 text-rose-700'}`}>{message}</p> : null}
